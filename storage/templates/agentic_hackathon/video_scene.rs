...
const VO_SRC: &str = "assets/agentic_hackathon_vo.mp3";
const SFX_WHOOSH: &str = "assets/whoosh.mp3";
const SFX_CLICK: &str = "assets/universal-click.mp3";
const BG_MUSIC: &str = "stock-assets/nastelbom-upbeat-upbeat-background.mp3";
...
fn svg_zap(c: &str) -> String {
    format!(
        r##"<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fill="none"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z" stroke="{c}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>"##
    )
}
fn svg_arrow_right(c: &str) -> String {
    format!(
        r##"<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" fill="none"><path d="M5 12h14M12 5l7 7-7 7" stroke="{c}" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/></svg>"##
    )
}
// Memory: concentric expanding rings ("memory layers")
fn svg_memory(c: &str) -> String {
    format!(
        r##"<svg viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" fill="none"><circle cx="32" cy="32" r="6" fill="{c}"/><circle cx="32" cy="32" r="14" stroke="{c}" stroke-width="2.5"/><circle cx="32" cy="32" r="22" stroke="{c}" stroke-width="2" opacity="0.7"/><circle cx="32" cy="32" r="30" stroke="{c}" stroke-width="1.5" stroke-dasharray="3 4" opacity="0.45"/></svg>"##
    )
}
...
fn svg_sparkle(c: &str) -> String {
    format!(
        r##"<svg viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" fill="none"><path d="M32 4 L36 28 L60 32 L36 36 L32 60 L28 36 L4 32 L28 28 Z" fill="{c}"/><path d="M52 8 L54 16 L62 18 L54 20 L52 28 L50 20 L42 18 L50 16 Z" fill="{c}" opacity="0.7"/></svg>"##
    )
}
// Trophy
...
fn svg_skyline(c: &str) -> String {
    format!(
        r##"<svg viewBox="0 0 200 80" xmlns="http://www.w3.org/2000/svg" fill="none"><path d="M0 80 L0 60 L20 60 L20 40 L34 40 L34 56 L48 56 L48 30 L56 22 L64 30 L64 56 L82 56 L82 36 L96 36 L96 50 L120 50 L120 24 L132 12 L144 24 L144 50 L168 50 L168 44 L184 44 L184 80 Z" fill="{c}"/><circle cx="132" cy="32" r="3" fill="{c}"/></svg>"##
    )
}
...
fn svg_person(c: &str) -> String {
    format!(
        r##"<svg viewBox="0 0 24 32" xmlns="http://www.w3.org/2000/svg" fill="none"><circle cx="12" cy="8" r="5.5" fill="{c}"/><path d="M2 32C2 22 6 18 12 18C18 18 22 22 22 32" fill="{c}"/></svg>"##
    )
}
...
fn build_act1(scene: &mut Scene) -> Id {
    ...
    for (i, (sx, sy, scol)) in shift_specs.iter().enumerate() {
        ...
        comp.animate(sq)
            ...
            .kf_eased(
                1.20,
                AnimatedProperty::rotation_z(180.0),
                Easing::EASE_IN_OUT,
            )
    ...
    comp.animate(ring)
        ...
        .kf_eased(
            act1_out + 0.30,
            AnimatedProperty::rotation_z(34.0),
            Easing::EASE_IN_OUT,
        )
    ...
    comp.animate(halo)
        ...
        .kf_eased(
            act1_out + 0.30,
            AnimatedProperty::rotation_z(-26.0),
            Easing::EASE_IN_OUT,
        )
        .kf(act1_out, AnimatedProperty::opacity(1.0))
        .kf_eased(
...
fn build_act2(scene: &mut Scene) -> Id {
    ...
    comp.animate(agent_label)
        ...
        .kf(
            4.32,
            AnimatedProperty::position(agent_x - cap_w * 0.5, agent_y + 38.0),
        )
        .kf_eased(
            5.10,
    ...
    comp.animate(anchor)
        ...
        .kf(
            4.05,
            AnimatedProperty::position(anchor_x - anchor_size * 0.5, anchor_y - anchor_size * 0.5),
        )
        .kf_eased(
            4.12,
    ...
    comp.animate(agent)
        ...
        .kf(
            4.05,
            AnimatedProperty::position(agent_x - agent_r, agent_y - agent_r),
        )
        .kf_eased(
            4.15,
    ...
    fn build_metaphor_beat(
        ...
        comp.animate(icon)
            ...
            .kf_eased(
                t_in + 0.78,
                AnimatedProperty::rotation_z(4.0),
                Easing::EASE_IN_OUT,
            )
            .kf_eased(
                t_out - 0.20,
...
fn build_act4(scene: &mut Scene) -> Id {
    ...
    fn build_pure_metaphor(
        ...
        comp.animate(icon)
            ...
            .kf_eased(
                t_in + 0.78,
                AnimatedProperty::rotation_z(3.0),
                Easing::EASE_IN_OUT,
            )
            .kf_eased(
                t_out - 0.20,
        ...
        let ring = comp
            ...
            .at(
                cx - (icon_size + 40.0) * 0.5,
                cy - (icon_size + 40.0) * 0.5 - 30.0,
            )
            .depth(0.112)
            .add();
        comp.animate(ring)
            ...
            .kf_eased(
                t_out,
                AnimatedProperty::rotation_z(20.0),
                Easing::EASE_IN_OUT,
            )
            .kf(t_out - 0.10, AnimatedProperty::opacity(1.0))
            .kf_eased(t_out, AnimatedProperty::opacity(0.0), Easing::EASE_IN)
...
fn build_act5(scene: &mut Scene) -> Id {
    ...
    comp.animate(cross)
        ...
        .kf_eased(
            27.40,
            AnimatedProperty::rotation_z(90.0),
            Easing::EASE_IN_OUT,
        )
        .kf(lockup_out - 0.18, AnimatedProperty::opacity(1.0))
        .kf_eased(lockup_out, AnimatedProperty::opacity(0.0), Easing::EASE_IN)
    ...
    comp.animate(mongodb.clone())
        ...
        .kf(
            pour_t0,
            AnimatedProperty::position(lockup_cx - gap, cy - 60.0),
        )
        .kf_eased(
            pour_t1,
    ...
    comp.animate(cv.clone())
        ...
        .kf(
            pour_t0,
            AnimatedProperty::position(cv_final_x - cv_w, cy - 60.0),
        )
        .kf_eased(
            pour_t1,
...
fn build_act6(scene: &mut Scene) -> Id {
    ...
    fn build_prize_beat(
        ...
        comp.animate(icon)
            ...
            .kf_eased(
                t_in + 0.85,
                AnimatedProperty::rotation_z(6.0),
                Easing::EASE_IN_OUT,
            )
            .kf_eased(
                t_in + 1.20,
        ...
        comp.animate(ring)
            ...
            .kf_eased(
                t_out,
                AnimatedProperty::rotation_z(15.0),
                Easing::EASE_IN_OUT,
            )
            .kf(t_out - 0.12, AnimatedProperty::opacity(1.0))
            .kf_eased(t_out, AnimatedProperty::opacity(0.0), Easing::EASE_IN)
    ...
    build_prize_beat(
        &mut comp, cx, cy, "£15,000", 148.0, amber, HEX_AMBER, svg_trophy, 29.83, 31.85,
    );
    build_prize_beat(
        &mut comp,
        cx,
...
fn build_scene() -> Scene {
    ...
    let (inst1, inst2, inst3, inst4, inst5, inst6, inst7, inst8) = {
        ...
        let flash_times: &[f32] = &[2.35, 9.55, 14.45, 24.85, 29.72, 41.00, 45.75];
        ...
        let eyebrow = comp
            ...
            .depth(0.026)
            .add();
        comp.animate(eyebrow).fade_in(0.80, 0.60).ease_out().apply();
